...
// Auth.js's Resend provider calls this (passed in as a `sendVerificationRequest`
// override in auth.js) instead of its own default unbranded template, so the
// magic-link email matches the rest of the site's notifications.
export async function sendMagicLinkEmail({ identifier: to, url }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    console.error(
      "RESEND_API_KEY is not set — cannot send magic-link sign-in email. " +
        "Set it in the deployment environment to enable sign-in."
    );
  ...
  const html = `
    <h2>Sign in to ${SITE.name}</h2>
    <p>Click the link below to sign in. This link expires in 24 hours and can only be used once.</p>
    <p><a href="${url}" style="display:inline-block;padding:12px 24px;background:#1a1a1a;color:#fdfaf4;text-decoration:none;border-radius:6px;">Sign in to ${host}</a></p>
